...
import { BaseControl, Button, Card, CardBody, CardHeader, PanelBody, PanelRow, SelectControl, __experimentalSpacer as Spacer, __experimentalToggleGroupControl as ToggleGroupControl, __experimentalToggleGroupControlOption as ToggleGroupControlOption, ToggleControl } from '@wordpress/components';
...
export const LpFilterOptions = props => {
  ...
  return (
    ...
    >
      ...
      {(nglue_backend.license_tier === 'new_pro' || nglue_backend.license_tier === 'new_basic') && (
      <PanelRow>
        <ToggleControl
          label={__('Insert Posts from RSS', 'newsletter-glue')}
          checked={attributes.insertRssPosts || false}
          onChange={(value) => setAttributes({ insertRssPosts: value })}
        />
      </PanelRow>
      )}
      ...
      {attributes.insertRssPosts && (nglue_backend.license_tier === 'new_pro' || nglue_backend.license_tier === 'new_basic') && (
        <p className="components-base-control__help">
          The RSS feed you insert below will be used to insert the posts in the latest posts block from an external source.
        </p>
      )}
      ...
      {attributes.insertRssPosts && (nglue_backend.license_tier === 'new_pro' || nglue_backend.license_tier === 'new_basic') && (
        <PanelRow>
          <BaseControl
            label={__('RSS Feed', 'newsletter-glue')}
            id="ngl-select-rssfeed"
          >
            <input
              type="text"
              id="ngl-select-rssfeed"
              value={attributes.rssfeed}
              onChange={(e) => {
                setAttributes({ rssfeed: e.target.value });
              }}
            />
          </BaseControl>
        </PanelRow>
      ...
      <PanelRow>
        <BaseControl
          label={__('Number of posts', 'newsletter-glue')}
          id="ngl-posts-num"
        >
          <input
            type="number"
            ...
            min="1"
            onChange={(e) => {
              setAttributes({ posts_num: parseInt(e.target.value) });
            }}
          />
        </BaseControl>
      </PanelRow>
      ...
      <PanelRow>
        <BaseControl
          label={__('Offset (skip posts)', 'newsletter-glue')}
          id="ngl-posts-offset"
          help={__('Number of posts to skip. Useful for excluding the most recent posts.', 'newsletter-glue')}
        >
          <input
            type="number"
            id="ngl-posts-offset"
            ...
            min="0"
            onChange={(e) => {
              setAttributes({ offset: parseInt(e.target.value) });
            }}
          />
        </BaseControl>
      </PanelRow>
