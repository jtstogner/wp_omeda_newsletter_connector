...
function newsletterglue_duplicate_post_excludelist_filter( $meta_excludelist ) {
	...
	return array_merge( $meta_excludelist, [ '_ngl_results', '_ngl_core_pattern', '_ngl_core_template' ] );
...
function newsletterglue_get_current_post_type() {
	...
	elseif( isset( $_GET['post_type'] ) ) // phpcs:ignore
...
function newsletterglue_wp_stackable_hide_dynamic_content_button_on_ngl_post_type() {
	if ( in_array( newsletterglue_get_current_post_type(), array( 'newsletterglue', 'ngl_pattern' ) ) ) {
		?>

		<style>
			.wp-admin.newsletterglue-ui .stackable-dynamic-content{
				display: none;
			}
		</style>
...
add_action( 'admin_head', 'newsletterglue_wp_stackable_hide_dynamic_content_button_on_ngl_post_type' );
