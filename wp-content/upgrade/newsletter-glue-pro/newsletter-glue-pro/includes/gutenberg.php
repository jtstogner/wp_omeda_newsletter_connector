...
function newsletterglue_get_blocks() {
	...
	$blocks = array(
		...
		'newsletterglue_block_columns'           => array(),
		'newsletterglue_block_latest_posts'      => array(),
	);
...
add_filter( 'block_categories_all', 'newsletterglue_add_block_category', 10, 2 );
...
function newsletterglue_load_frontend_scripts( $hook ) {
	...
	if ( ! empty( $post->ID ) ) {
		if ( strstr( $post->post_content, 'newsletterglue' ) ) {
			$load = true;
		}
	}
	...
	wp_localize_script(
		'newsletterglue_gutenberg',
		'newsletterglue_gutenberg',
		apply_filters(
			'newsletterglue_gutenberg_js_params',
			array(
