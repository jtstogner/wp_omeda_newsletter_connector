...
/**
 * Show a checkbox or toggle field.
 */
function newsletterglue_checkbox_field( $args ) {
    $id             = isset( $args['id'] ) ? $args['id'] : '';
    $name           = isset( $args['name'] ) ? $args['name'] : $id;
    $label          = isset( $args['label'] ) ? $args['label'] : '';
    $helper         = isset( $args['helper'] ) ? $args['helper'] : '';
    ...
    ?>
    <div class="field <?php echo strstr( $class, 'ngl-ajax' ) ? 'ngl-ajax-field' : ''; ?>">
        <div class="ngl-toggle-wrapper" style="display: flex; align-items: center; margin-bottom: 8px;">
            <?php if ( $label ) { ?>
            <label for="<?php echo esc_attr( $id ); ?>" style="margin-right: 10px;"><?php echo esc_html( $label ); ?></label>
            <?php } ?>
            <div class="ui toggle checkbox <?php echo esc_attr( $class ); ?>">
                <input type="checkbox" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $id ); ?>" value="1" <?php echo $checked; ?> <?php echo esc_attr( $disabled ); ?>>
