...
function newsletterglue_add_category_column( $columns ) {
	...
	if ( ! empty( $ngl_columns ) && isset( $ngl_columns[ 'tags' ] ) ) {
		unset( $ngl_columns['tags'] );
	}
	...
	switch ( $column ) {
		...
		case 'ngl_tag' :
			...
			if ( ! empty( $terms ) ) {
				$output = '';
				foreach( $terms as $term ) {
					$output .= '<a href="' . admin_url( 'edit.php?post_type=newsletterglue&post_tag=' . $term->slug ) . '">' . $term->name . '</a> (<a href="' . admin_url( 'term.php?taxonomy=post_tag&tag_ID=' . $term->term_id ) . '">' . __( 'Edit', 'newsletter-glue' ) . '</a>)<span style="display:inline-block;width:20px;"></span>';
				}
				echo wp_kses_post( $output );
			} else {
				echo '&mdash;';
 ...
 * Add newsletter tag column.
 */
function newsletterglue_add_newsletter_cat_column( $columns ) {

	foreach( $columns as $key => $value ) {
		$ngl_columns[ $key ] = $value;
		if ( $key == 'title' ) {
			$ngl_columns[ 'ngl_pattern_category' ] = __( 'Pattern category', 'newsletter-glue' );
		}
	}
...
add_filter( "manage_ngl_pattern_posts_columns", 'newsletterglue_add_newsletter_cat_column', 200 );

/**
 * Display custom post columns.
 */
function newsletterglue_display_newsletter_pattern_column( $column, $post_id ) {

	switch ( $column ) {
		case 'ngl_pattern_category' :

			$terms = wp_get_post_terms( $post_id, 'ngl_pattern_category' );
			if ( ! empty( $terms ) ) {
				$output = '';
				foreach( $terms as $term ) {
					$output .= '<a href="' . admin_url( 'edit.php?post_type=ngl_pattern&ngl_pattern_category=' . $term->slug ) . '">' . $term->name . '</a> (<a href="' . admin_url( 'term.php?taxonomy=ngl_pattern_category&tag_ID=' . $term->term_id . '&post_type=newsletterglue' ) . '">' . __( 'Edit', 'newsletter-glue' ) . '</a>)<span style="display:inline-block;width:20px;"></span>';
				}
				echo wp_kses_post( $output );
			} else {
				echo '&mdash;';
...
add_action( "manage_ngl_pattern_posts_custom_column", 'newsletterglue_display_newsletter_pattern_column', 99, 2 );
