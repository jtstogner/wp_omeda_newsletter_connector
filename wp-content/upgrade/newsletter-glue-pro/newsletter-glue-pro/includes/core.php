...
function newsletterglue_add_title( $title, $post ) {
	...
	// Get filterable h1 text alignment (default: left)
	$h1_text_align = apply_filters( 'newsletterglue_title_h1_text_align', 'left', $post );
	
	return apply_filters( 'newsletterglue_post_title', '<h1 class="title" style="text-align: ' . esc_attr( $h1_text_align ) . ';">' . $title . '</h1>', $post );
}
...
function newsletterglue_generate_content( $post = '', $subject = '', $app = '' ) {
	...
	if ( $has_custom_header ) {
		if ( apply_filters( 'newsletterglue_post_title_url', true ) ) {
			// Get filterable h1 text alignment (default: center)
			$h1_text_align = apply_filters( 'newsletterglue_title_h1_text_align', 'left', $post );
			
			// Get filterable anchor text decoration (default: none)
			$anchor_text_decoration = apply_filters( 'newsletterglue_title_anchor_text_decoration', 'underline', $post );
			
			$the_content .= apply_filters( 'newsletterglue_the_post_title', '<h1 class="title" style="text-align: ' . esc_attr( $h1_text_align ) . '!important;"><a style="text-decoration: ' . esc_attr( $anchor_text_decoration ) . ';" href="' . get_permalink( $post->ID ) . '">' . $post->post_title . '</a></h1>', $post );
		} else {
			// Get filterable h1 text alignment (default: left)
			$h1_text_align = apply_filters( 'newsletterglue_title_h1_text_align', 'left', $post );
			
			$the_content .= '<h1 class="title" style="text-align: ' . esc_attr( $h1_text_align ) . ';">' . $post->post_title . '</h1>';
		}
	}
