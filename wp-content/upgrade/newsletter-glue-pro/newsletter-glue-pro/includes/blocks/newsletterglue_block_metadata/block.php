...
class NGL_Block_Metadata extends NGL_Abstract_Block {
	...
	public function get_demo_url() {
		return 'https://www.youtube.com/embed/ddMFgYY0OnA?autoplay=1&modestbranding=1&autohide=1&showinfo=0&controls=0';
	}
	...
	public function render_block( $attributes, $content ) {
		...
		if ( empty( $post ) ) {
			if ( isset( $_GET[ 'preview_email' ] ) ) { // phpcs:ignore
		...
		if ( defined( 'NGL_IN_EMAIL' ) && $content ) {
			$content = $this->tableize( $content, $post_id );
		}
	...
	public function tableize( $content, $post_id = 0 ) {
		...
		foreach( $output->find( $replace ) as $key => $element ) {
			...
			if ( strstr( $element->class, 'ngl-metadata-date' ) ) {
				$format = ! empty( $element->{ 'data-date-format' } ) ? $element->{ 'data-date-format' } : '';
				$innertext = '<span class="auto_date" data-post-id="' . $post_id . '" data-date-format="' . $format . '">' . $innertext . '</span>';
			}
			if ( $i == 1 ) {
				$output->find( $replace, $key )->outertext = $innertext . '&nbsp;';
			} else {
				$output->find( $replace, $key )->outertext = '&nbsp;' . $innertext . '&nbsp;';
			}
		}
		...
		foreach( $output->find( $replace ) as $key => $element ) {
			...
			$html .= '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="ngl-table ngl-table-metadata"><tr><td valign="middle" align="' . $align . '">' . $element->innertext . '</td></tr></table>';
