( function( blocks, editor, element, components ) {
	...
	const { TextControl, SelectControl, ToggleControl, Panel, PanelBody, PanelRow, RangeControl, BaseControl, ButtonGroup, Button, __experimentalBoxControl, CheckboxControl } = components;
	...
	registerBlockType( 'newsletterglue/form', {
		...
		keywords: [ 'newsletter', 'form', 'subscribe' ],
		attributes: {
			message_text: {
				'type' : 'string',
		...
		edit: withColors( 'formColor' ) ( function( props ) {
			...
			if ( ! app ) {
				...
				var showForm = el( 'div', { className: 'ngl-form' + ' ' + isPortraitclass, style: metaStyles, 'data-padding': top_padding + ',' + bottom_padding + ',' + left_padding + ',' + right_padding, 'data-margin' : top_margin + ',' + bottom_margin },
						addHeading,
						addDescription,
			...
			var makeNameRequired = '';
			if ( props.attributes.add_name ) {
				makeNameRequired = el( BaseControl, { className: 'ngl-sub-cb' }, 
					el( CheckboxControl, {
						label: "Make it required",
						checked: props.attributes.name_required,
						onChange: ( value ) => { props.setAttributes( { name_required: value } ); },
					} )
				);
			}
			...
			if ( props.attributes.add_checkbox ) {
				makeCbRequired = el( BaseControl, { className: 'ngl-sub-cb' }, 
					el( CheckboxControl, {
						label: "Make it required",
						checked: props.attributes.cb_required,
						onChange: ( value ) => { props.setAttributes( { cb_required: value } ); },
					} )
				);
			}
		...
		save: function( props, className ) {
			...
			// Add name.
			if ( props.attributes.add_name ) {
				var nameAttrs = { type: 'text', className: 'ngl-form-input-text', name: 'ngl_name', id: 'ngl_name', placeholder: props.attributes.name_placeholder, style: { borderRadius: props.attributes.form_radius } };

				if ( props.attributes.name_required ) {
					nameAttrs.required = 'required';
				}
			...
			if ( props.attributes.add_checkbox && props.attributes.form_style !== 'landscape' ) {
				...
				var checkboxAttrs = { type: 'checkbox', name: 'ngl_extra_list', id: 'ngl_extra_list' };

				if ( props.attributes.cb_required ) {
					checkboxAttrs.required = 'required';
				}
			...
			var formEmail = el( 'div', { className: 'ngl-form-field', style: fieldStyle },
							...
							el( 'div', { className: 'ngl-form-input' },
								el( 'input', { type: 'email', required: 'required', className: 'ngl-form-input-text', name: 'ngl_email', id: 'ngl_email', placeholder: props.attributes.email_placeholder, style: { borderRadius: props.attributes.form_radius } },
			...
			return (
					...
					el( 'form', { className: 'ngl-form' + ' ' + isPortraitclass, action: '', method: 'post', metaStyles, 'data-padding': top_padding + ',' + bottom_padding + ',' + left_padding + ',' + right_padding, 'data-margin' : top_margin + ',' + bottom_margin },
						formHeader,
						formDescription,
						el( 'div', { className: 'ngl-form-container' },
							formName,
