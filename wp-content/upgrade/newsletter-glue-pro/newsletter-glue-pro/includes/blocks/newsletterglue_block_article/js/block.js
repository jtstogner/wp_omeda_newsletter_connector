( function( blocks, editor, element, components ) {
	...
	registerBlockType( 'newsletterglue/article', {
		...
		edit: withColors( 'formColor' ) ( function( props ) {
			...
			if ( typenow && ( typenow == 'ngl_pattern' || typenow == 'ngl_template' ) ) {
				var scoped = 'pattern';
			} else {
				var scoped = 'regular';
			}
			...
			setTimeout( () => {
				if ( props && ! props.attributes.block_id ) {
					props.setAttributes( { block_id: props.clientId } );
					props.setAttributes( { scope: scoped } );
				}
			}, 1500 );
			...
			var show_excerpt = props.attributes.show_excerpt;

			if ( show_excerpt ) {
				var showReadMore = el( BaseControl, {},
								el( ToggleControl, {
									label: 'Show read more link',
									onChange: ( value ) => { props.setAttributes( { show_more_link: value } ); },
			...
			if ( props.attributes.show_more_link ) {
				var showReadMoreText = el( BaseControl, {},
								el( TextControl, {
									label: 'Read more link text',
									onChange: ( value ) => { props.setAttributes( { read_more_text: value } ); },
								} )
						);
			} else {
				var showReadMoreText = '';
			}
					...
					el( InspectorControls, {},
						...
						el( PanelBody, { title: 'Post embed options', initialOpen: true },
							...
							el( BaseControl, {},
								el( ToggleControl, {
									label: 'Show excerpt',
									onChange: ( value ) => { props.setAttributes( { show_excerpt: value } ); },
