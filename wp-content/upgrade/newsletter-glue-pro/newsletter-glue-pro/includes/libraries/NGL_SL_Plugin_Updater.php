...
class NGL_SL_Plugin_Updater {
    ...
    'slug'      => $this->slug,
    'beta'      => $this->beta,
    'item_name' => $this->api_data['item_name'], // <-- Add this
    'license'   => $this->api_data['license'],   // <-- Add this too (optional but recommended)
    'version'   => $this->api_data['version'],   // <-- Add this too (optional)
    'author'    => $this->api_data['author'],    // <-- Add this too (optional)
));
	...
	public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
		...
		$cache_key = 'edd_api_request_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) ); // phpcs:ignore
	...
	private function api_request( $_action, $_data ) {
		...
		if ( ! is_array( $edd_plugin_url_available ) || ! isset( $edd_plugin_url_available[ $store_hash ] ) ) {
			...
			if ( empty( $host ) ) {
				...
				$response = wp_remote_get( $test_url, array( 'timeout' => $this->health_check_timeout, 'sslverify' => $verify_ssl ) ); // phpcs:ignore
		...
		$request    = wp_remote_post( $this->api_url, array( 'timeout' => 15, 'sslverify' => $verify_ssl, 'body' => $api_params ) ); // phpcs:ignore
