...
/**
 * Send automated emails immediately when requested.
 */
add_action( 'newsletterglue_trigger_automated_email_send_now', 'newsletterglue_trigger_automated_email_send_now', 10 );
function newsletterglue_trigger_automated_email_send_now( $post_id ) {
	ngl_debug_log( "Starting immediate send for post ID: $post_id", 'SEND_NOW' );

	$run = true;

	$automation = new NGL_Automation( $post_id );
	ngl_debug_log( 'Automation object created for immediate send', 'SEND_NOW' );

	// Fail. Post unpublished.
	if ( $automation->get_post_status() !== 'publish' ) {
		ngl_debug_log( "Automation post status is not 'publish'. Status: " . $automation->get_post_status(), 'SEND_NOW_FAIL' );
		$run = false;
	}

	// Fail. Paused.
	if ( ! $automation->is_enabled() ) {
		ngl_debug_log( 'Automation is not enabled', 'SEND_NOW_FAIL' );
		$run = false;
	}

	if ( ! $run ) {
		ngl_debug_log( "Clearing scheduled hook for post ID: $post_id", 'SEND_NOW_HOOK' );
		wp_clear_scheduled_hook( 'newsletterglue_trigger_automated_email_send_now', array( $post_id ) );
	} else {
		ngl_debug_log( "Proceeding with immediate send for post ID: $post_id", 'SEND_NOW_PROCESS' );

		$send_type = $automation->get_send_type();
		ngl_debug_log( "Send type: $send_type", 'SEND_NOW_PROCESS' );

		// Log the settings and query parameters for Latest Posts block.
		$settings = newsletterglue_get_data( $post_id );
		ngl_debug_log( 'Automation settings for immediate send:', 'SEND_NOW_SETTINGS' );
		ngl_debug_log( $settings, 'SEND_NOW_SETTINGS' );

		// Create the newsletter.
		ngl_debug_log( 'Creating WordPress newsletter for immediate send', 'SEND_NOW_CREATE' );
		$campaign_id = $automation->create_wp_newsletter();
		ngl_debug_log( "Created newsletter with campaign ID: $campaign_id", 'SEND_NOW_CREATE' );

		// Get the campaign content to check if Latest Posts block is working correctly.
		$campaign_post = get_post( $campaign_id );
		if ( $campaign_post ) {
			ngl_debug_log( "Campaign title: {$campaign_post->post_title}", 'SEND_NOW_CONTENT' );

			// Check for Latest Posts block in content
			if ( strpos( $campaign_post->post_content, 'wp:newsletterglue/latest-posts' ) !== false || strpos( $campaign_post->post_content, 'wp:newsletterglue/latestposts' ) !== false ) {
				ngl_debug_log( 'Latest Posts block found in campaign content', 'SEND_NOW_CONTENT' );
			} else {
				ngl_debug_log( 'No Latest Posts block found in campaign content', 'SEND_NOW_CONTENT' );
			}
		}

		// Send a campaign?
		if ( 'send' === $send_type ) {
			ngl_debug_log( "Sending campaign ID: $campaign_id immediately", 'SEND_NOW_SEND' );
			$result = $automation->send_campaign( $campaign_id );
			ngl_debug_log( 'Send result:', 'SEND_NOW_SEND' );
			ngl_debug_log( $result, 'SEND_NOW_SEND' );

			ngl_debug_log( "Creating log entry for campaign ID: $campaign_id", 'SEND_NOW_LOG' );
			$automation->create_log( $campaign_id, $result );

			ngl_debug_log( "Removing WordPress post for campaign ID: $campaign_id", 'SEND_NOW_CLEANUP' );
			$automation->remove_wp_post( $campaign_id );
		}

		// Clear the send_now flag to prevent repeated sends
		update_post_meta( $post_id, '_send_now', '' );
	}

	ngl_debug_log( "Completed immediate send process for post ID: $post_id", 'SEND_NOW_COMPLETE' );
}
