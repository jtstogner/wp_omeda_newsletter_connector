...
class NGL_Automation {
	 ...
	 * Get send now.
	 */
	public function get_send_now() {
		return get_post_meta( $this->id, '_send_now', true );
	}

	/**
	 * Set send now.
	 */
	public function set_send_now( $value ) {
		update_post_meta( $this->id, '_send_now', $value );
	}
	...
	public function enable() {
		...
		$send_now = $this->get_send_now();

		if ( ( $send_now ) ) {
			error_log( 'Send now' );
			$should_send_now = get_post_meta( $this->id, '_send_now', true );

			if ( ! ( $should_send_now ) ) {
				error_log( 'Show not send now' );
				wp_clear_scheduled_hook( 'newsletterglue_trigger_automated_email_send_now', array( $this->id ) );
				error_log( 'Send now removed' );
				return;
			}
			error_log( 'Should send now' );
			$datetime->modify( '+15 seconds' );
			$next_run_date  = $datetime->format( 'Y-m-d H:i:s' );
			$next_run       = strtotime( $gmt_offset, strtotime( $next_run_date ) );
			update_post_meta( $this->id, '_send_now', $next_run_date );
			error_log( 'Send now set' );
			wp_schedule_single_event( $next_run, 'newsletterglue_trigger_automated_email_send_now', array( $this->id ) );
			$this->set_send_now( false );
			error_log( 'Send now scheduled' );
		} elseif( wp_next_scheduled( 'newsletterglue_trigger_automated_email_send_now', array( $this->id ) ) ) {
			wp_clear_scheduled_hook( 'newsletterglue_trigger_automated_email_send_now', array( $this->id ) );
		}
			...
			if ( ! empty( $blocks[1] ) ) {
				...
				foreach ( $found as $key => $data ) {
					if ( strstr( $data, '"posts":[' ) ) {
						...
						$clean_content = str_replace('u0022', '\"', $clean_content);
						file_put_contents( $log_file, 'Clean Content after stripslashes and str_replace: ' . $clean_content . PHP_EOL, FILE_APPEND );
					...
					if($args['words_num'] > 0) {
						...
						file_put_contents( $log_file, "Processing full content mode" . PHP_EOL, FILE_APPEND );
						file_put_contents( $log_file, 'Clean Content before json_decode: ' . $clean_content . PHP_EOL, FILE_APPEND );
						$decoded_content = json_decode($clean_content, true);
						file_put_contents( $log_file, 'Decoded Content: ' . print_r( $decoded_content, true ) . PHP_EOL, FILE_APPEND );
						...
						if (is_array($decoded_content)) {
							foreach ($decoded_content as &$post_item) {
								if (isset($post_item['post_content'])) {
									...
									// Preserve newlines properly - this is critical
									$post_item['post_content'] = str_replace("\r\n", "\n", $post_item['post_content']);
							...
							$clean_content = wp_json_encode($decoded_content, JSON_UNESCAPED_UNICODE | JSON_HEX_QUOT | JSON_HEX_TAG);
							file_put_contents( $log_file, 'Re-encoded Content: ' . $clean_content . PHP_EOL, FILE_APPEND );
							...
							//$clean_content = str_replace('\\', '\\\\', $clean_content);
							file_put_contents( $log_file, 'Clean Content after str_replace: ' . $clean_content . PHP_EOL, FILE_APPEND );
						}
					}
