...
class Omeda_Workflow_Manager {
    ...
    public function create_and_assign_audience($post_id, $config_id) {
        ...
        try {
            ...
            $this->log_status($post_id, "Step 1/4 Complete: Deployment created with TrackID: {$track_id}");
            ...
            $this->log_status($post_id, 'Step 3/4 Complete: Content and subject sent to Omeda.');

            // Step 4: Log deployment date info
            $this->log_status($post_id, "Step 4/4 Complete: Initial deployment setup complete. Scheduled for {$next_hour_date} UTC (temporary date - will update when published).");
    ...
    // --- Logging Helpers ---
    public function log_status($post_id, $message) {
        $this->add_to_workflow_log($post_id, $message, 'INFO');
    }

    public function log_warning($post_id, $message) {
        $this->add_to_workflow_log($post_id, $message, 'WARN');
    }

    public function log_error($post_id, $error_message, $context = null) {
