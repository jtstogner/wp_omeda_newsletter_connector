...
( function( $ ) {
	...
	$( document ).on( 'click', '.ngl-article-featured-edit i.image.icon', function( event ) {
	...
	$( document ).on( 'click', '.ngl-article-featured-edit i.trash.icon', function( event ) {
	...
	$( document ).on( 'focus', '.ngl-article-excerpt[contenteditable]', function() {
		const $this = $(this);
		$this.data('before', $this.html());
	}).on( 'blur', '.ngl-article-excerpt[contenteditable]', function() {
		const $this = $(this);
		if ($this.data('before') !== $this.html()) {
			$this.data('before', $this.html());
			$this.trigger('change');
	...
	$( document ).on( 'focus', '.ngl-article-title span[contenteditable]', function() {
		const $this = $(this);
		$this.data('before', $this.html());
	}).on( 'blur', '.ngl-article-title span[contenteditable]', function() {
		const $this = $(this);
		if ($this.data('before') !== $this.html()) {
			$this.data('before', $this.html());
			$this.trigger('change');
	...
	function updateEmbeds() {
		...
		inp.each( function() {
			...
			if ( typenow && scope == 'pattern' && typenow != 'ngl_pattern' && typenow != 'ngl_template' ) {
	...
	// Fix blocks.
	function fixBlocks() {
    var el = $( '.components-tab-panel__tab-content .editor-block-list-item-newsletterglue-group' );
    if ( el.length ) {
      el.closest( '.block-editor-inserter__panel-content' ).remove();
    }
    ...
    let blocks = [ "author", "group", "form", "article", "callout", "metadata", "share", "columns" ];
    $.each( blocks, function(key, value) {
      $( 'button.editor-block-list-item-newsletterglue-' + value ).parent().hide();
      $( 'button[id$="-block-newsletterglue/' + value + '"]' ).hide();
    } );
	}
