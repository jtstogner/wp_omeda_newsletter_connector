...
function newsletterglue_uninstall( $network_wide = false ) {
	...
	// Delete options.
	$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'newsletterglue\_%';" ); // phpcs:ignore
	$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE '%\_newsletterglue\_%';" ); // phpcs:ignore
	$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'ngl_articles\_%';" ); // phpcs:ignore
	$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'ngl_embed\_%';" ); // phpcs:ignore

	// Delete usermeta.
	$wpdb->query( "DELETE FROM {$wpdb->usermeta} WHERE meta_key LIKE '_newsletterglue\_%';" ); // phpcs:ignore
	$wpdb->query( "DELETE FROM {$wpdb->usermeta} WHERE meta_key LIKE '%\_newsletterglue\_%';" ); // phpcs:ignore
	$wpdb->query( "DELETE FROM {$wpdb->usermeta} WHERE meta_key LIKE '%\_ngl\_%';" ); // phpcs:ignore
...
function newsletterglue_install_roles_on_network() {
	...
	if ( ! get_option( 'newsletterglue_updated_capabilities_v5' ) ) {
		newsletterglue_assign_caps();
		update_option( 'newsletterglue_updated_capabilities_v5', 'yes' );
	}

	if ( isset( $_GET[ 'uninstall-newsletterglue' ] ) ) { // phpcs:ignore
		if ( current_user_can( 'manage_newsletterglue' ) ) {
			newsletterglue_uninstall();
		}
	}

	if ( isset( $_GET[ 'install-newsletterglue-roles' ] ) ) { // phpcs:ignore
		if ( current_user_can( 'manage_newsletterglue' ) ) {
			newsletterglue_assign_caps();
		}
	}
...
function newsletterglue_onboarding_screen() {
	...
	if ( isset( $_GET[ 'skip_onboarding' ] ) ) { // phpcs:ignore
		update_option( 'newsletterglue_onboarding_done', 'yes' );
	}

	if ( current_user_can('manage_newsletterglue') && isset($_GET['ng-recreate-welcome-post']) && 'yes' === $_GET['ng-recreate-welcome-post'] ) {
		newsletterglue_create_demo_post();
	}

	// Setup wizard redirect.
	if ( get_transient( '_ngl_onboarding' ) ) {
		$do_redirect  = true;
		$current_page = isset( $_GET['ngl_screen'] ) ? sanitize_text_field( wp_unslash( $_GET['ngl_screen'] ) ) : false; // phpcs:ignore
		...
		if ( 'onboarding' === $current_page || isset( $_GET['activate-multi'] ) ) { // phpcs:ignore
			delete_transient( '_ngl_onboarding' );
			$do_redirect = false;
		}
		...
		if ( $do_redirect ) {
			...
			if ( $demo_post ) {
				wp_safe_redirect( admin_url( 'admin.php?page=ngl-welcome-wizard' ) );
				exit;
			}
		} else {
			// Redirect to templates page.
			delete_transient( 'ngl_onboarding' );
			if ( ! get_option( 'newsletterglue_template_wizard_done' ) ) {
				wp_safe_redirect( admin_url( 'edit.php?post_type=ngl_pattern' ) );
				exit;
			}
		}
...
function newsletterglue_create_demo_post() {
	...
	if ( $demo_post ) {
		...
		if ( isset( $post->ID ) && in_array( $post->post_status, array( 'draft', 'publish', 'trash' ) ) ) {
			return $demo_post;
		}
	}

	$content = newsletterglue_get_onboarding_post();
 ...
 */
function newsletterglue_assign_caps() {
	global $wp_roles;
	...
	$admin_caps = array(
		...
		'publish_newsletterglue',
		'add_newsletterglue',
		'edit_newsletterglue',
		'manage_newsletterglue_patterns',
	);
	...
	foreach( $admin_caps as $cap ) {
		...
		$wp_roles->add_cap( 'newsletterglue_manager', $cap );
	}

	// Allow editor to send newsletter.
	$wp_roles->add_cap( 'editor', 'publish_newsletterglue' );
	$wp_roles->add_cap( 'editor', 'edit_newsletterglue' );
	$wp_roles->add_cap( 'editor', 'add_newsletterglue' );

	// Misc.
	$wp_roles->add_cap( 'author', 'edit_newsletterglue' );
	$wp_roles->add_cap( 'contributor', 'edit_newsletterglue' );
}

/**
 * Updated user permissions for specific role.
 */
function newsletterglue_assign_caps_for_role( $role = '' ) {
	global $wp_roles;
	...
	$all_caps = array(
		...
		'edit_newsletterglue',
		'manage_newsletterglue_patterns',
		'create_newsletterglues',
	);

	// Reset given role first.
	foreach( $all_caps as $cap ) {
		$wp_roles->remove_cap( $role, $cap );
	}

	if ( $role == 'administrator' || $role == 'newsletterglue_manager' ) {
		foreach( $all_caps as $cap ) {
			$wp_roles->add_cap( 'administrator', $cap );
			$wp_roles->add_cap( 'newsletterglue_manager', $cap );
		}
