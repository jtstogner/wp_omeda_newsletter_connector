...
if (!class_exists('CS_REST_Events')) {
    class CS_REST_Events extends CS_REST_Wrapper_Base {
        ...
        $transport = NULL) {
            ...
            if (!isset($event_type)) {
                trigger_error('$event_type should be one of \'custom\', \'identify\' or \'shopify\''); // phpcs:ignore
            }
        ...
        function set_client_id($client_id) {
            if (!isset($client_id)) {
                trigger_error('$client_id needs to be set'); // phpcs:ignore
            }
        ...
        private function setEventType($event_type) {
            if (!isset($event_type)) {
                trigger_error('$event_type needs to be set'); // phpcs:ignore
            ...
            if (strcmp($event_type, "custom") !== 0 &&
                ...
                trigger_error('$event_type needs to be one of \'custom\', \'identify\' or \'shopify\''); // phpcs:ignore
        ...
        private function validateEmail($email) {
            if (!isset($email)) {
                trigger_error('$email needs to be set'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }

            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                trigger_error('$email needs to be a valid email address'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
        ...
        function track($email, $event_name, $anonymous_id = NULL, $user_id = NULL, $data = NULL)
        {
            // Basic validation
            if (!isset($event_name)) {
                trigger_error('$event_name needs to be set'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
            if (strlen($event_name) > 1000) {
                trigger_error('$event_name needs to be shorter, max length is 1000 bytes'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
            if (isset($data)) {
                if (!is_array($data)) {
                    trigger_error('$data needs to be a valid array'); // phpcs:ignore
                    return new CS_REST_Wrapper_Result(null, 400);
                }
            }
            if (empty($data)) {
            ...
            trigger_error('event type is invalid. Supported - custom, identify or shopify'); // phpcs:ignore
            return new CS_REST_Wrapper_Result(null, 400);
        }
        ...
        private function sendIdentifyTrack($email, $event_name, $anonymousId, $userId, $data) {
            if (!isset($email)) {
                trigger_error('email needs to be a set for identify event'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
            ...
            if ($paramPresent < $minRequiredParam) {
                trigger_error('at least one of: anonymous id, user id needs to be set and be a valid string for identify event'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
        ...
        private function sendNonIdentifyTrack($email, $event_name, $anonymousId, $userId, $data) {
            ...
            if ($paramPresent < $minRequiredParam) {
                trigger_error('at least one of: anonymous id, user id, email needs to be set and be a valid string for identify event'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
        ...
        private function sendTrack($payload) {
            if ($this->_invalid_event) {
                trigger_error('$event_type must be one of \'identify\', \'custom\' or \'shopify\''); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
            // Basic validation before finally POST'ing
            if (!isset($this->_base_route) || !isset($this->_event_type) || !isset($this->_client_id)) {
                trigger_error('one of: $_base_route, $_event_type, $_client_id is missing during URL construction'); // phpcs:ignore
                return new CS_REST_Wrapper_Result(null, 400);
            }
            ...
            trigger_error('$payload needs to be a valid array'); // phpcs:ignore
            return new CS_REST_Wrapper_Result(null, 400);
        }
    }
}
