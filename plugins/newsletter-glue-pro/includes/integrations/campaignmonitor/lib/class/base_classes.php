    ...
    class CS_REST_Wrapper_Base {
        ...
        function refresh_token() {
            if (!isset($this->_default_call_options['authdetails']) ||
                ...
                trigger_error( 'Error refreshing token. There is no refresh token set on this object.', E_USER_ERROR); // phpcs:ignore
            ...
            if ($result->was_successful()) {
                ...
                trigger_error( 'Error refreshing token. '.$result->response->error.': '.$result->response->error_description,  E_USER_ERROR); // phpcs:ignore
                return array(NULL, NULL, NULL);
            }
        }
        ...
        function _call($call_options, $method, $route, $data = NULL) {
            ...
            $this->_log->log_message('Call result: <pre>'.var_export($call_result, true).'</pre>', get_class($this), CS_REST_LOG_VERBOSE); // phpcs:ignore
