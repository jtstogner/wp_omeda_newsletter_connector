...
class CallbackFilter extends \php_user_filter
{
    ...
    public function filter($in, $out, &$consumed, $closing)
    {
        ...
        $data = '';
        while ($bucket = \stream_bucket_make_writeable($in)) { // phpcs:ignore
        ...
        if ($data !== '') {
            try {
                ...
                $this->onClose();
                \trigger_error('Error invoking filter: ' . $e->getMessage(), \E_USER_WARNING); // phpcs:ignore
        ...
        if ($closing) {
            ...
            if ($this->supportsClose) {
                ...
                try {
                    ...
                    \trigger_error('Error ending filter: ' . $e->getMessage(), \E_USER_WARNING); // phpcs:ignore
