...
class BufferedStream implements StreamInterface
{
    ...
    public function __construct(StreamInterface $stream, $useFileBuffer = true, $memoryBuffer = 2097152)
    {
        ...
        if ($useFileBuffer) {
            $this->resource = fopen('php://temp/maxmemory:'.$memoryBuffer, 'rw+'); // phpcs:ignore
        } else {
            $this->resource = fopen('php://memory', 'rw+'); // phpcs:ignore
        }
    ...
    public function read($length)
    {
        ...
        if (ftell($this->resource) !== $this->written) {
            $read = fread($this->resource, $length); // phpcs:ignore
        }
        ...
        if ($bytesRead < $length) {
            ...
            $this->written += fwrite($this->resource, $streamRead); // phpcs:ignore
            $read .= $streamRead;
        }
