...
\$j(document).ready(function() {
	...
	function process($params) {
		$r = array();
		if ( isset($_SERVER[ 'REQUEST_METHOD' ]) && $_SERVER["REQUEST_METHOD"] != "POST") return $r;

		// Check the nonce.
		if ( empty( $_POST['ngl_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ngl_form_nonce'] ) ), 'ngl_save_form' ) ) {
			return $r;
		}
		...
		$formid = isset( $_POST['f'] ) ? sanitize_text_field( wp_unslash( $_POST["f"] ) ) :null;
		// sub or unsub
		$act = isset($_POST["act"]) ? sanitize_text_field( wp_unslash( $_POST["act"] ) ) : "sub";
		...
		$lists = (isset($_POST["nlbox"]) && $_POST["nlbox"]) ? array_map( 'sanitize_key', $_POST["nlbox"] ) : array(); // phpcs:ignore
		if ($captcha_in_form) {
			// Captcha is part of the form.
			// Get the captcha value the user entered.
			$captcha = "";
			if (isset($_POST["captcha"])) {
				$captcha = sanitize_text_field( wp_unslash( $_POST["captcha"] ) );
				$captcha = md5(strtoupper((string) $captcha ));
			}
		...
		if (isset($_POST["fullname"])) {
			$fullname = explode(" ", sanitize_text_field( wp_unslash( $_POST["fullname"] ) ) );
		...
		elseif (isset($_POST["firstname"]) && isset($_POST["lastname"])) {
			...
			if ($firstname == "" && isset($_POST["first_name"])) $firstname = sanitize_text_field( wp_unslash($_POST["first_name"] ) );
			if ($lastname == "" && isset($_POST["last_name"])) $lastname = sanitize_text_field( wp_unslash($_POST["last_name"]) );
		}

		$fields = (isset($_POST["field"])) ? $_POST["field"] : array(); // phpcs:ignore
