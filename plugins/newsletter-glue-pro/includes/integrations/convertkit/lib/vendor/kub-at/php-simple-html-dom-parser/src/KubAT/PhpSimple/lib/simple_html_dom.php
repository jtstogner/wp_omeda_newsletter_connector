	...
	$defaultSpanText = DEFAULT_SPAN_TEXT)
{
	 ...
	 */
	$contents = file_get_contents( // phpcs:ignore
		$url,
		$use_include_path,
		$context,
		$offset,
...
class simple_html_dom_node
{
	...
	function dump($show_attr = true, $deep = 0)
	{
		...
		echo esc_html( $lead ) . esc_html( $this->tag );
...
class simple_html_dom
{
	...
	function save($filepath = '')
	{
		...
		if ($filepath !== '') { file_put_contents($filepath, $ret, LOCK_EX); } // phpcs:ignore
		return $ret;
	}
	...
	function restore_noise($text)
	{
		...
		while (($pos = strpos($text, '___noise___')) !== false) { // phpcs:ignore
