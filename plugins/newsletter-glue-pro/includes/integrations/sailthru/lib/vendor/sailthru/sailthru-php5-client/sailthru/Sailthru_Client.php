...
class Sailthru_Client {
    ...
    public function setHorizonCookie($email, $domain = null, $duration = null, $secure = false) {
        ...
        if (!$domain) {
            $domain_parts = isset($_SERVER['HTTP_HOST']) ? explode('.', sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) ) : array();
        ...
        return setcookie('sailthru_hid', $data['keys']['cookie'], $expire, '/', $domain, $secure); // phpcs:ignore
    }
    ...
    protected function httpRequestWithoutCurl($action, $data, $method = 'POST', $options = [ ]) {
        ...
        $fp = @fopen($url, 'rb', false, $ctx); // phpcs:ignore
        if (!$fp) {
            throw new Sailthru_Client_Exception(
