...
class NGL_Mailchimp extends NGL_Abstract_Integration {
	...
	public function __construct() {
		...
		add_filter( 'newsletterglue_email_end_mailchimp', array( $this, 'newsletterglue_email_end_mailchimp' ), 10, 2 );
	...
	public function add_integration( $args = array() ) {
		...
		if ( ! $valid_account ) {
			...
			// Check if this is a different account than previously connected
			$is_new_connection = ! $this->already_integrated( $this->app, $api_key );

			if ( $is_new_connection ) {
				// For new connections, save the integration
				$this->save_integration( $api_key, $account );
				
				// Clear any existing audience data for previous accounts
				$this->clear_audience_data();
			} else {
				// For existing connections, just refresh the audience data
				$this->refresh_audience_data();
			}

			// Always refresh audience data for successful connections
			$this->refresh_audience_data();
	...
	public function save_integration( $api_key = '', $account = '' ) {
		...
		// Get existing integrations or initialize as empty array
		...
		// Add default options.
		$globals = get_option( 'newsletterglue_options', array() );
		
		// Initialize the app array if it doesn't exist
		if ( !isset($globals[$this->app]) || !is_array($globals[$this->app]) ) {
	...
	public function get_segments( $audience_id = '' ) {
		...
		$data = $this->api->get(
			...
			array(
				'count'  => apply_filters( 'newsletterglue_mailchimp_segments_count', 1000 ),
	...
	public function send_newsletter( $post_id = 0, $data = array(), $test = false ) {
		...
		$campaign_array['settings']['from_name'] = html_entity_decode( $from_name, ENT_QUOTES, 'UTF-8' );
	...
	public function prepare_message( $result ) {
		...
		if ( isset( $result['status'] ) ) {
			...
			if ( $result['status'] == 400 ) {
				$output['status']  = 400;
				$output['type']    = 'error';
				$output['message'] = __( 'Bad request - check your MailChimp account', 'newsletter-glue' );
				$output['help']    = '';
			}
			...
			if ( $result['status'] == 404 ) {
				...
				$output['notice']  = sprintf(
					__( 'Your email newsletter was not sent, because your email address is not verified. %1$s Or %2$s', 'newsletter-glue' ),
					'<a href="https://admin.mailchimp.com/account/domains/" target="_blank" class="ngl-link-inline-svg">' . __( 'Verify email now', 'newsletter-glue' ) . ' <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M18.2 17c0 .7-.6 1.2-1.2 1.2H7c-.7 0-1.2-.6-1.2-1.2V7c0-.7.6-1.2 1.2-1.2h3.2V4.2H7C5.5 4.2 4.2 5.5 4.2 7v10c0 1.5 1.2 2.8 2.8 2.8h10c1.5 0 2.8-1.2 2.8-2.8v-3.6h-1.5V17zM14.9 3v1.5h3.7l-6.4 6.4 1.1 1.1 6.4-6.4v3.7h1.5V3h-6.3z"></path></svg></a>',
					'<a href="https://newsletterglue.com/docs/email-verification-my-email-is-not-verified/" target="_blank">' . __( 'learn more.', 'newsletter-glue' ) . '</a>'
				);
				$output['help']    = 'https://newsletterglue.com/docs/email-verification-my-email-is-not-verified/';
			}
	...
	public function add_user( $data ) {
		...
		if ( isset( $extra_list ) && ! empty( $extra_list_id ) ) {
			$batch->put(
				"op$extra_list_id",
				"lists/$extra_list_id/members/$hash",
				array(
					'email_address' => $email,
					'status'        => $double_optin,
					'status_if_new' => $double_optin,
	 ...
	 * Customize content.
	 */
	public function newsletterglue_email_end_mailchimp( $content, $post ) {
		...
		$unsub = str_replace( '{{ unsubscribe_link }}', '*|UNSUB|*', $unsub );
	...
	public function get_custom_fields() {
		...
		if ( is_array( $audiences ) ) {
			...
			foreach ( $audiences as $audience_id => $audience_name ) {
				$response = $this->api->get( 'lists/' . $audience_id . '/merge-fields', array(
                    'count' => apply_filters( 'newsletterglue_mailchimp_merge_fields_count', 1000 ),
            ...
            // Get Interests
			foreach ( $audiences as $audience_id => $audience_name ) {
				$response = $this->api->get( 'lists/' . $audience_id . '/interest-categories', array(
                    'count' => apply_filters( 'newsletterglue_mailchimp_interest_categories_count', 1000 ),
                    'fields' => 'categories.title',
                ));
				if ( ! empty( $response ) && isset( $response['categories'] ) ) {
					foreach ( $response['categories'] as $key => $array ) {
						if ( isset( $array['title'] ) && ! empty( $array['title'] ) ) {
							$_fields[] = array(
								'label' => 'Group: ' . $array['title'],
								'value' => 'Group:' . $array['title'],
							);
						}
					}
				}
	 ...
	 * Clear audience data.
	 * 
	 * This method clears any existing audience data for the current ESP
	 * when connecting a new account.
	 */
	public function clear_audience_data() {
		try {
			// Get the current options
			$globals = get_option( 'newsletterglue_options', array() );
			
			// Make sure globals is an array
			if ( !is_array($globals) ) {
				$globals = array();
			}
			
			// Initialize or reset the app array
			$globals[$this->app] = array();
			...
			// Log error if needed
			error_log( 'Newsletter Glue - Error clearing audience data: ' . $e->getMessage() );
		}
	}
	 ...
	 * 
	 * This method fetches the latest audiences and segments from Mailchimp
	 * and updates the WordPress options accordingly.
	 */
	public function refresh_audience_data() {
		try {
			// Connect to the API
			$this->connect();
			...
			// Get the default audience ID
			$default_audience_id = $this->get_default_list_id();
			
			// Update the audience option
			if ( $default_audience_id ) {
				$globals[$this->app]['audience'] = $default_audience_id;
				
				// Get segments for this audience
				$segments = $this->get_segments( $default_audience_id );
				
				// Make sure segments is an array
				if ( is_array($segments) && !empty($segments) ) {
					$segment_keys = array_keys( $segments );
					$globals[$this->app]['segment'] = isset($segment_keys[0]) ? $segment_keys[0] : '_everyone';
				} else {
					$globals[$this->app]['segment'] = '_everyone';
				}
			}
			...
			// Log error if needed
			error_log( 'Newsletter Glue - Error refreshing audience data: ' . $e->getMessage() );
		}
	}
		...
		foreach ( $output->find( $replace ) as $key => $element ) {
			...
			foreach ( $conditions as $condition ) {
                ...
                if( substr($key, 0, 6) === 'Group:' ) {
                    ...
                    if ( $operator == 'eq' ) {
                        $contentStart .= "*|INTERESTED:$key:$value|*";
                    } elseif ( $operator == 'neq' ) {
                        $contentStart .= "*|INTERESTED:$key:$value|* *|ELSE:|*";
                    }

                    $contentStart .= ' ';
                    if( ! empty( trim( $contentStart ) ) ) {
                        $contentEnd    = " *|END:INTERESTED|*$contentEnd";
                    }
