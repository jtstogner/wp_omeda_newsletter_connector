...
class NGL_Salesforce extends NGL_Abstract_Integration {
	...
	public function __construct() {
		...
		// Register hook for adding tracking pixel before closing body tag
		add_action( 'newsletterglue_email_before_closing_body', array( $this, 'add_tracking_pixel_before_body_close' ) );

		// Read, Write permission is required for ExactTargetWSDL.xml
		@chmod( dirname( __FILE__ ) . '/lib', 0777 ); // Silenced to prevent permission errors
	...
	public function add_integration( $args = array() ) {
		...
		// Try to validate the connection using multiple endpoints in order of preference
		$account = null;
		
		// First try: Get organization info (original method)
		try {
			...
			error_log( 'Salesforce Newsletter Glue: Organization API failed: ' . $e->getMessage() );
		}
		
		// Second try: Get sender profiles if organization info failed
		if ( ! $account ) {
			try {
				$profiles = $this->api->getSenderProfiles();
				if ( ! empty( $profiles ) ) {
					// If we get any profiles, the API is working
					$account = array( 'ID' => 'verified_by_sender_profiles' );
				}
			} catch ( Exception $e ) {
				// Log the error but continue to try other methods
				error_log( 'Salesforce Newsletter Glue: Sender Profiles API failed: ' . $e->getMessage() );
			}
		}
		
		// Third try: Get send classifications if previous methods failed
		if ( ! $account ) {
			try {
				$classifications = $this->api->getSendClassifications();
				if ( ! empty( $classifications ) ) {
					// If we get any classifications, the API is working
					$account = array( 'ID' => 'verified_by_send_classifications' );
				}
			} catch ( Exception $e ) {
				// Log the error but continue
				error_log( 'Salesforce Newsletter Glue: Send Classifications API failed: ' . $e->getMessage() );
			}
		}
		
		// Fourth try: Just check if we can get the client instance
		if ( ! $account ) {
			try {
				$client = $this->api->getClient();
				if ( $client ) {
					// If we can get a client instance, the credentials are at least valid
					$account = array( 'ID' => 'verified_by_client_connection' );
				}
			} catch ( Exception $e ) {
				// Log the error
				error_log( 'Salesforce Newsletter Glue: Client connection failed: ' . $e->getMessage() );
			}
		}
		...
		if ( ! $account ) {
			...
			error_log( 'Salesforce Newsletter Glue: Invalid credentials' );
			error_log( 'Salesforce Newsletter Glue: Validated by ' . $account[ 'ID' ] );
	...
	public function html_content( $html, $post_id ) {
		...
		$html = $this->add_tracking_pixel( $html );
	 ...
	 * Add tracking pixel.
	 */
	public function add_tracking_pixel( $html ) {
		// We'll keep this method for backward compatibility, but it won't add the pixel here
		// The pixel will be added via the newsletterglue_email_before_closing_body hook instead
		return $html;
	}
	 ...
	 */
	public function add_tracking_pixel_before_body_close() {
		echo '<!-- Salesforce Marketing Cloud tracking pixel -->';
