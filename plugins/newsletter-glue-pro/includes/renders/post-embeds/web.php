...
<div class="<?php echo esc_attr( $classes ); ?>">
	...
	<div class="ngl-lp-items" style="<?php echo esc_attr( implode( '; ', $styles[ 'container' ] ) ); ?>">
		<?php foreach( $embeds_order as $index ) : if ( ! isset( $posts[$index] ) ) continue; $item = $posts[$index]; ?>
				...
				foreach( $order as $index ) :
					...
					if ( $index === 3 && ! empty( $show_heading ) ) {
						...
						<div class="ngl-lp-title">
							<h3 style="<?php echo esc_attr( implode( '; ', $styles[ 'heading' ] ) ); ?>"><a href="<?php echo esc_url( $item[ 'permalink' ] ); ?>" style="<?php echo esc_attr( implode( '; ', $styles[ 'heading' ] ) ); ?>;padding: 0px;"><?php echo wp_kses_post( $item[ 'post_title' ] ); ?></a></h3>
						</div>
						<?php
					}
