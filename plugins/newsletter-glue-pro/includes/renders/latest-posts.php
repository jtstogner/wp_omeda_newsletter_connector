...
class NGL_Render_Latest_Posts {
	...
	public function setup_attrs( $atts ) {
		...
		if (defined('NGL_IN_EMAIL')) {
			...
			file_put_contents( $log_file, "\nContent Options in setup_attrs: " . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "show_label: " . ($args['show_label'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "show_author: " . ($args['show_author'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "show_heading: " . ($args['show_heading'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "show_excerpt: " . ($args['show_excerpt'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "show_cta: " . ($args['show_cta'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "show_image: " . $args['show_image'] . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, "Original attributes: " . print_r($atts, true) . PHP_EOL, FILE_APPEND );
		}
		...
		if (defined('NGL_IN_EMAIL')) {
			...
			file_put_contents($log_file, "\nDivider Debug at " . date('Y-m-d H:i:s') . ": \n", FILE_APPEND);
			file_put_contents($log_file, "show_divider isset: " . (isset($atts['show_divider']) ? 'yes' : 'no') . "\n", FILE_APPEND);
			if (isset($atts['show_divider'])) {
				file_put_contents($log_file, "show_divider value: " . var_export($atts['show_divider'], true) . "\n", FILE_APPEND);
				file_put_contents($log_file, "show_divider type: " . gettype($atts['show_divider']) . "\n", FILE_APPEND);
			}
			file_put_contents($log_file, "divider_size isset: " . (isset($atts['divider_size']) ? 'yes' : 'no') . "\n", FILE_APPEND);
			if (isset($atts['divider_size'])) {
				file_put_contents($log_file, "divider_size value: " . var_export($atts['divider_size'], true) . "\n", FILE_APPEND);
			}
		}
		...
		if (isset($atts['show_divider']) && $atts['show_divider'] == true) {
			$args['divider_size'] = !empty($atts['divider_size']) ? absint($atts['divider_size']) : 1;
			if (defined('NGL_IN_EMAIL')) {
				file_put_contents($log_file, "Setting divider_size to: " . $args['divider_size'] . "\n", FILE_APPEND);
			}
		} else {
			$args['divider_size'] = null;
			if (defined('NGL_IN_EMAIL')) {
				file_put_contents($log_file, "Setting divider_size to null\n", FILE_APPEND);
			}
		}
	...
	public function get_posts( $args ) {
		...
		if ( isset( $cached_results[ $block_key ] ) ) {
			file_put_contents( $log_file, "\n\n======== USING CACHED RESULTS AT " . date( 'Y-m-d H:i:s' ) . " ========\n", FILE_APPEND );
			file_put_contents( $log_file, 'Block Key: ' . $block_key . " | Using cached results\n", FILE_APPEND );
		...
		file_put_contents( $log_file, "\n\n======== NEW RENDER AT " . date( 'Y-m-d H:i:s' ) . " ========\n", FILE_APPEND );
		file_put_contents( $log_file, 'Block Key: ' . $block_key . ' | Render Count: ' . $render_count[ $block_key ] . PHP_EOL, FILE_APPEND );
		...
		file_put_contents( $log_file, 'Original Posts Num: ' . print_r( $posts_num, true ) . PHP_EOL, FILE_APPEND );
		...
		if ( ! empty( $offset ) && $offset > 0 ) {
			...
			file_put_contents( $log_file, 'Adjusted Posts Num +1: ' . print_r( $adjusted_posts_num, true ) . PHP_EOL, FILE_APPEND );
		}

		file_put_contents( $log_file, 'Final Adjusted Posts Num: ' . print_r( $adjusted_posts_num, true ) . PHP_EOL, FILE_APPEND );
		...
		file_put_contents( $log_file, 'Query Args: ' . print_r( $query_args, true ) . PHP_EOL, FILE_APPEND );
		...
		if ( ! empty( $offset ) && $offset > 0 && ! empty( $posts ) && ! $offset_applied[ $block_key ] ) {
			file_put_contents( $log_file, "Applying offset for the first time: offset = $offset, original_posts_num = $original_posts_num\n", FILE_APPEND );
			$posts = array_slice( $posts, $offset, $original_posts_num );
			$offset_applied[ $block_key ] = true;
		} else if ( ! empty( $offset ) && $offset > 0 ) {
			file_put_contents( $log_file, "Skipping offset application as it was already applied or not needed\n", FILE_APPEND );
		}
		...
		file_put_contents( $log_file, 'Offset with Array_Slice: ' . print_r( $log_posts, true ) . PHP_EOL, FILE_APPEND );
		...
		if ( ! empty( $posts ) ) {
			...
			file_put_contents( $log_file, 'Results to be returned: ' . print_r( $results, true ) . PHP_EOL, FILE_APPEND );
	...
	public function render_block_web( $block_content, $block ) {
		...
		file_put_contents( $log_file, "\n\n======== NEW WEB RENDER AT " . date( 'Y-m-d H:i:s' ) . " ========\n", FILE_APPEND );
		file_put_contents( $log_file, 'Block Key: ' . $block_key . ' | Render Count: ' . $render_web_count[ $block_key ] . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, 'Call Stack: ' . print_r( $trace, true ) . PHP_EOL, FILE_APPEND );
		...
		if ( $render_web_count[ $block_key ] > 1 ) {
			file_put_contents( $log_file, "SKIPPING DUPLICATE WEB RENDER FOR THIS BLOCK\n", FILE_APPEND );
	...
	public function render_block_mail( $block_content, $block ) {
		...
		if ( isset( $processed_blocks[ $block_key ] ) ) {
			file_put_contents( $log_file, "\n\n======== SKIPPING ALREADY PROCESSED BLOCK AT " . date( 'Y-m-d H:i:s' ) . " ========\n", FILE_APPEND );
			file_put_contents( $log_file, 'Block Key: ' . $block_key . " | Already processed\n", FILE_APPEND );
...
/*
		...
		if ( isset( $block['attrs']['posts'] ) && ! empty( $block['attrs']['posts'] ) ) {
			file_put_contents( $log_file, "Using posts from block attributes instead of making a new query\n", FILE_APPEND );
			...
			if ( isset( $block['attrs']['custom_data'] ) && ! empty( $block['attrs']['custom_data'] ) ) {
				file_put_contents( $log_file, "Applying custom_data edits to posts from attributes\n", FILE_APPEND );
				...
				foreach ( $block['attrs']['custom_data'] as $key => $entry ) {
					foreach ( $entry as $subdata => $data_in ) {
						if ( $data_in ) {
							...
							$post_id = str_replace( '_', '', $datatype[1] );
							$custom[ $post_id ][ $data_is ] = $data_in;
							file_put_contents( $log_file, "Found custom data: {$data_is} for post {$post_id}\n", FILE_APPEND );
						}
					}
				}
				...
				foreach ( $posts as $key => $post ) {
					...
					if ( isset( $custom[ $post_id ]['title'] ) ) {
						...
						file_put_contents( $log_file, "Applied custom title for post {$post_id}: {$custom[ $post_id ]['title']}\n", FILE_APPEND );
					}
					
					// Apply image edit
					if ( isset( $custom[ $post_id ]['image'] ) ) {
						$posts[$key]['featured_image'] = $custom[ $post_id ]['image'];
						file_put_contents( $log_file, "Applied custom image for post {$post_id}\n", FILE_APPEND );
					}
					
					// Apply label edit
					if ( isset( $custom[ $post_id ]['label'] ) ) {
						$posts[$key]['labels'] = $custom[ $post_id ]['label'];
						file_put_contents( $log_file, "Applied custom label for post {$post_id}: {$custom[ $post_id ]['label']}\n", FILE_APPEND );
					}
					
					// Apply author edit
					if ( isset( $custom[ $post_id ]['author'] ) ) {
						$posts[$key]['author'] = $custom[ $post_id ]['author'];
						file_put_contents( $log_file, "Applied custom author for post {$post_id}: {$custom[ $post_id ]['author']}\n", FILE_APPEND );
					}
					...
					if ( isset( $custom[ $post_id ]['excerpt'] ) ) {
						...
						file_put_contents( $log_file, "Applied custom excerpt for post {$post_id}\n", FILE_APPEND );
					}
					
					// Apply CTA link edit
					if ( isset( $custom[ $post_id ]['ctalink'] ) ) {
						$posts[$key]['cta_link'] = $custom[ $post_id ]['ctalink'];
						file_put_contents( $log_file, "Applied custom CTA link for post {$post_id}: {$custom[ $post_id ]['ctalink']}\n", FILE_APPEND );
					}
				}
			}
			...
			if ( isset( $args['hidden'] ) && !empty( $args['hidden'] ) ) {
				file_put_contents( $log_file, "Filtering out hidden posts from attributes: " . print_r( $args['hidden'], true ) . PHP_EOL, FILE_APPEND );
				$filtered_posts = array();
				foreach ( $posts as $post ) {
					if ( !in_array( $post['id'], $args['hidden'] ) ) {
						$filtered_posts[] = $post;
					} else {
						file_put_contents( $log_file, "Hiding post ID: " . $post['id'] . " (" . $post['post_title'] . ")\n", FILE_APPEND );
					}
				}
			...
			file_put_contents( $log_file, 'Processing posts from attributes to ensure all required keys exist' . PHP_EOL, FILE_APPEND );
			file_put_contents( $log_file, 'Label type: ' . $label_type . PHP_EOL, FILE_APPEND );
			
			// Log the first post structure before processing
			if (!empty($posts)) {
				file_put_contents( $log_file, 'First post structure before processing: ' . print_r( $posts[0], true ) . PHP_EOL, FILE_APPEND );
			}
			...
			if (!empty($posts)) {
				file_put_contents( $log_file, 'First post structure after processing: ' . print_r( $posts[0], true ) . PHP_EOL, FILE_APPEND );
			}
			...
			file_put_contents( $log_file, 'Posts from attributes (after hidden filtering): ' . print_r( $log_posts, true ) . PHP_EOL, FILE_APPEND );
		} else {
			// If no posts in attributes, get them using the standard method.
			file_put_contents( $log_file, "No posts in block attributes, fetching fresh posts\n", FILE_APPEND );
			$posts = $this->get_posts( $args );
		}

		if ( empty( $posts ) ) {
			file_put_contents( $log_file, "No posts found, returning null\n", FILE_APPEND );
			return null;
		}
		...
		if (isset($block['attrs']['show_divider']) && $block['attrs']['show_divider'] === true) {
			...
			file_put_contents( $log_file, "Forcing divider to show with size: {$divider_size}px\n", FILE_APPEND );
		} else {
		...
		file_put_contents( $log_file, "\nDivider debug in render_block_mail: " . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "divider_size (before extract): " . var_export($divider_size, true) . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "divider_bg (before extract): " . var_export($divider_bg, true) . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "is_full (before extract): " . var_export($is_full, true) . PHP_EOL, FILE_APPEND );

		// Log the content options just before rendering the email
		file_put_contents( $log_file, "\nContent Options in render_block_mail before extract: " . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_label: " . ($args['show_label'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_author: " . ($args['show_author'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_heading: " . ($args['show_heading'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_excerpt: " . ($args['show_excerpt'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_cta: " . ($args['show_cta'] ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_image: " . $args['show_image'] . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "divider_size: " . (isset($args['divider_size']) ? $args['divider_size'] : 'null') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "divider_bg: " . $args['divider_bg'] . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "is_full: " . $args['is_full'] . PHP_EOL, FILE_APPEND );
		...
		file_put_contents( $log_file, "\nContent Options in render_block_mail after extract: " . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_label: " . ($show_label ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		
		// Log divider variables after extract
		file_put_contents( $log_file, "divider_size after extract: " . (isset($divider_size) ? var_export($divider_size, true) : 'not set') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "divider_size empty check: " . (!empty($divider_size) ? 'not empty' : 'empty') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "divider_bg after extract: " . (isset($divider_bg) ? $divider_bg : 'not set') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "is_full after extract: " . (isset($is_full) ? $is_full : 'not set') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_author: " . ($show_author ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_heading: " . ($show_heading ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_excerpt: " . ($show_excerpt ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_cta: " . ($show_cta ? 'true' : 'false') . PHP_EOL, FILE_APPEND );
		file_put_contents( $log_file, "show_image: " . $show_image . PHP_EOL, FILE_APPEND );
